...
-- | curry converts a function of pairs to a curried function (of two arguments).
curry :: (PairClass p) => (p a b -> c) -> a -> b -> c
curry f a b = f (pair a b)

-- | Extract the first component of a pair.
fst :: (PairClass p) => p a b -> a
fst pereche = uncurry (\x y -> x) pereche
...
snd pereche = uncurry (\x y -> y) pereche
...
-- instance (Show a, Show b) => Show (CPair a b) where
--   show cp = "<" <> show (uncurry pair (getCPair cp)) <> ">"
